...
## 17. 카운트다운 UX 안정화 업데이트 (2026-03-21)

### 이번 창에서 유지한 핵심 전제
- 실제 플레이 대상은 스마트폰이다.
- Windows는 개발/테스트용이다.
- 시간 판정은 반드시 서버가 한다.
- 수정은 최신 첨부 파일 기준으로만 작은 단위 diff로 진행한다.

### 이번 창에서 완료된 사항
#### 1) 369 시작 직후 즉시 입력되는 급한 느낌 완화
- `game:start` 직후 바로 숫자 입력이 시작되는 대신,
  시작 전 준비 단계(`waiting_start`)를 거치도록 정리했다.
- 시작 준비 구간에서는 시각적 카운트다운(`3 → 2 → 1 → 시작`)을 표시한 뒤 실제 입력 단계로 넘어가도록 했다.
- 실제 턴 제한시간 시작 시점은 서버 기준으로 유지한다.

#### 2) 카운트다운 표시 위치 안정화
- 처음 시도했던 “중앙 큰 시작 준비 박스” 방식은 제거했다.
- 이유: 카운트다운 박스가 사라질 때 키패드 위치가 위로 당겨져 사용자가 당황할 수 있었기 때문이다.
- 최종 방식은 상단 우측의 기존 정답 표시 박스 자리를 재사용하는 구조다.
- 즉, 카운트다운 중에는 그 자리에서만 `3 / 2 / 1 / 시작`을 보여주고,
  카운트다운 종료 후에는 다시 정답 숫자 표시로 복귀한다.

#### 3) 카운트다운과 정답 숫자 시각 구분 강화
- 카운트다운 숫자(`3 / 2 / 1`)는 빨간색으로 표시한다.
- `시작` 텍스트는 초록색으로 표시한다.
- 실제 정답 숫자는 기존 검정색을 유지한다.
- 목적: 시작 직전 카운트다운과 실제 제출해야 할 정답 숫자가 헷갈리지 않도록 하기 위함이다.

#### 4) 입력 영역 비활성 시각 표현 강화
- `waiting_start` 동안 하단 숫자패드 전체를 흐리게 보이도록 조정했다.
- 입력창에는 `시작 준비 중` 텍스트를 표시해 현재 입력할 수 없는 상태임을 명확히 했다.
- 비활성 버튼 색도 더 분명한 회색 계열로 조정했다.
- 즉, “눌러도 반응이 없다”가 아니라 “지금은 아직 입력할 수 없다”가 한눈에 보이도록 개선했다.

#### 5) 이번 창에서 해결된 개발 흐름상 문제
- `main.dart`에서 카운트다운 표시 관련 positional argument / unused element / named parameter 혼선 문제를 정리했다.
- 최신 첨부 파일 기준으로만 다시 diff를 맞춰 적용하는 원칙을 재확인했다.
- 결과적으로 UI 흔들림 없이 시작 흐름을 안정화했다.

### 현재 369 게임 상태 요약 (이번 창 종료 기준)
...
- 시작 전 카운트다운 표시 추가 완료
- 카운트다운 표시 위치를 상단 우측 고정 구조로 안정화 완료
- 카운트다운/정답 숫자 색 구분 완료
- 카운트다운 중 입력 패널 비활성 시각 표현 완료

### 아직 이번 창에서 하지 않은 것
- 만세 입력 추가
- 난이도 선택 UI 추가
- 서버 `moveType` 확장
- 369 엔진의 난이도별 실제 판정 규칙 확장
- 음성 시작 구호 또는 효과음 추가

### 다음 창 우선순위 (이번 창 종료 기준)
1. 최신 GitHub 저장소 구조 재확인
2. 최신 설계문서 기준 현재 단계 재확인
3. 최신 첨부 파일 기준으로만 작업 시작
4. `만세 입력` 추가 가능 여부 확인
5. `난이도 선택 UI` 설계 및 최소 서버 타입 확장 확인
6. 서버 `moveType` 확장 가능 여부 확인
7. 필요 파일:
   - `app/flutter_app/lib/main.dart`
   - `server/src/games/types.ts`
   - `server/src/games/engines/ThreeSixNineEngine.ts`
